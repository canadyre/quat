...
print(mods)
attack_mods = ['frcnn', 'ssd300', 'yolo'] # , 'ssd300', 'yolo'
...
for m in mods:
    ...
    for at in attac:
        for bn in bnd:
            #print((m,'frcnn',at,bn))
            #print(df.first().loc[(m,'frcnn',at,bn)]['mAP50'])
...
plt.tight_layout()
plt.savefig('./plots/jetson_frcnn_attacks.png')
#plt.show()
plt.clf()
...
for m in mods:
    ...
    if ((m == 'yolo5l-n-clean') | (m == 'yolo5l-n-adv') | (m == 'yolo5s-n-clean') | (m == 'yolo5s-n-adv')):
        continue
    #print(df.first().loc[(m,'clean','clean', 0)]['mAP50'])
    map.append(df.first().loc[(m,'clean','clean', 0)]['mAP50'])
    att_array.append('clean')
    # for a in attack_mods:
    for at in attac:
        for bn in bnd:
            #print((m,'ssd300',at,bn))
...
plt.title('SSD300')
plt.tight_layout()
plt.savefig('./plots/jetson_ssd300_attacks.png')
#plt.show()
...
for m in mods:
    ...
    for at in attac:
        for bn in bnd:
            #print((m,'yolo',at,bn))
            #print(df.first().loc[(m,'yolo',at,bn)])
            #print(df.first().loc[(m,'yolo',at,bn)]['mAP50'])
...
plt.title('YOLO')
plt.tight_layout()
plt.savefig('./plots/jetson_yolo_attacks.png')
#plt.show()
