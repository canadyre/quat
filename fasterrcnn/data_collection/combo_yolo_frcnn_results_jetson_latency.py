...
yolo_results = '/home/canadyre/main_Dropbox/IC2E_code/mAP/jetson_test.csv'
yolo_results_time = '/home/canadyre/main_Dropbox/IC2E_code/time_per_image_new_jetson.csv'
...
frcnn_results = '/home/canadyre/adv_fusion/results/jetson_results.csv'
...
#print(df.first().loc[('yolov3-full-adv','fabrication','32')]['eval_time'])
...
for m in mods:
    ...
    if m[:4] == 'yolo':
        combo_latency = float(df.first().loc[(m,'clean','clean', 0)]['eval_time']
                            + df.first().loc[(m,'clean','clean', 0)]['eval_time']
                            + df.first().loc[(m,'clean','clean', 0)]['eval_time'])
    else:
        combo_latency = float(df.first().loc[(m,'clean','clean', 0)]['eval_time']
                            + df.first().loc[(m,'clean','clean', 0)]['eval_time']
                            + df.first().loc[(m,'clean','clean', 0)]['eval_time']
                            + df.first().loc[(m,'clean','clean', 0)]['pre-process']
                            + df.first().loc[(m,'clean','clean', 0)]['pre-process']
                            + df.first().loc[(m,'clean','clean', 0)]['pre-process'])
    map.append(combo_latency/3)
    att_array.append('clean')
    # for a in attack_mods:
    for at in attac:
        for bn in bnd:
            #print(df.first().loc[(m,'*',at,bn)]['eval_time'])
            if m[:4] == 'yolo':
                combo_latency = float(df.first().loc[(m,'frcnn',at,bn)]['eval_time']
                                    + df.first().loc[(m,'ssd300',at,bn)]['eval_time']
                                    + df.first().loc[(m,'ssd300',at,bn)]['eval_time'])
            else:
                combo_latency = float(df.first().loc[(m,'frcnn',at,bn)]['eval_time']
                                    + df.first().loc[(m,'ssd300',at,bn)]['eval_time']
                                    + df.first().loc[(m,'ssd300',at,bn)]['eval_time']
                                    + df.first().loc[(m,'ssd300',at,bn)]['pre-process']
                                    + df.first().loc[(m,'ssd300',at,bn)]['pre-process']
                                    + df.first().loc[(m,'ssd300',at,bn)]['pre-process'])
            map.append(combo_latency/3)
            att_array.append(str(at)+'_'+str(bn))
    plt.plot(att_array, map, label = m)
plt.legend(fontsize=7, ncol=2, loc='center', bbox_to_anchor=(0.5,0.38))
plt.xticks(rotation = 65)
plt.ylim()
plt.xlabel('Attack Type and Bound')
plt.ylabel('Pre-process + Evaluation Time (s)')
plt.title('Average of FasterRCNN, SSD300, YOLO on Jetson TX2')
plt.tight_layout()
plt.savefig('./plots/combo_attacks_latency_jetson.png')
#plt.show()
plt.clf()
...
#         continue
#     #print(df.first().loc[(m,'clean','clean', 0)]['mAP'])
#     map.append(df.first().loc[(m,'clean','clean', 0)]['eval_time'])
#     att_array.append('clean')
#     # for a in attack_mods:
#     for at in attac:
#         for bn in bnd:
#             #print(df.first().loc[(m,'*',at,bn)]['eval_time'])
#             combo_latency = df.first().loc[(m,'frcnn',at,bn)]['eval_time']
#             map.append(df.first().loc[(m,'frcnn',at,bn)]['eval_time'])
#             att_array.append(str(at)+'_'+str(bn))
#     plt.plot(att_array, map, label = m)
...
# plt.tight_layout()
# plt.savefig('./plots/frcnn_attacks_latency.png')
# #plt.show()
# plt.clf()
#
# for m in mods:
...
#             map.append(df.first().loc[(m,'ssd300',at,bn)]['eval_time'])
#             att_array.append(str(at)+'_'+str(bn))
#     plt.plot(att_array, map, label = m)
# plt.legend(fontsize=5)
# plt.xticks(rotation = 65)
...
# plt.title('SSD300')
# plt.tight_layout()
# plt.savefig('./plots/ssd300_attacks_latency.png')
# #plt.show()
#
# plt.clf()
#
...
#             map.append(df.first().loc[(m,'yolo',at,bn)]['eval_time'])
#             att_array.append(str(at)+'_'+str(bn))
#     plt.plot(att_array, map, label = m)
# plt.legend(fontsize=5)
# plt.xticks(rotation = 65)
...
# plt.title('YOLO')
# plt.tight_layout()
# plt.savefig('./plots/yolo_attacks_latency.png')
# #plt.show()
